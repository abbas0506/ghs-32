    ...
    <div class="space-y-4 pb-6">
        ...
        <div class="flex items-center justify-between py-1.5 border-b border-slate-100 pb-2">
            <div class="flex items-center gap-2">
                ...
                <span class="text-xs font-extrabold text-slate-800 tracking-tight uppercase">Edit NSB Receipt</span>
            </div>
        </div>
        ...
        <div class="bg-white rounded-xl border border-slate-100 shadow-sm max-w-md mx-auto overflow-hidden">
            <div class="px-4 py-3 border-b border-slate-50 bg-slate-50/50">
                ...
                <p class="text-[9px] text-slate-400 mt-0.5">Session: <span class="font-bold text-indigo-600">{{ $nsbReceipt->academicSession->name }}</span></p>
            </div>

            <form action="{{ route('nsb-receipts.update', $nsbReceipt->id) }}" method="POST" class="p-4 space-y-3">
                @csrf
                @method('PUT')
                ...
                <div class="space-y-1">
                    ...
                    <select name="quarter" id="quarter" class="w-full text-xs border border-slate-200 rounded-lg p-2 outline-none focus:border-indigo-500 transition-all bg-white" required>
                        ...
                        <option value="1" {{ old('quarter', $nsbReceipt->quarter) == 1 ? 'selected' : '' }}>Q1 (First Quarter)</option>
                        <option value="2" {{ old('quarter', $nsbReceipt->quarter) == 2 ? 'selected' : '' }}>Q2 (Second Quarter)</option>
                        <option value="3" {{ old('quarter', $nsbReceipt->quarter) == 3 ? 'selected' : '' }}>Q3 (Third Quarter)</option>
                        <option value="4" {{ old('quarter', $nsbReceipt->quarter) == 4 ? 'selected' : '' }}>Q4 (Fourth Quarter)</option>
                    </select>
                </div>
                ...
                <div class="space-y-1">
                    ...
                    <div class="relative rounded-lg overflow-hidden">
                        <input type="number" name="amount" id="amount" value="{{ old('amount', $nsbReceipt->amount) }}" placeholder="Enter amount" min="1" class="w-full text-xs border border-slate-200 rounded-lg p-2 outline-none focus:border-indigo-500 transition-all" required>
                ...
                <div class="space-y-1">
                    ...
                    <input type="date" name="received_date" id="received_date" value="{{ old('received_date', $nsbReceipt->received_date->toDateString()) }}" class="w-full text-xs border border-slate-200 rounded-lg p-2 outline-none focus:border-indigo-500 transition-all" required>
                </div>

                <!-- Description / Notes -->
                <div class="space-y-1">
                    <label for="description" class="text-[10px] font-bold text-slate-550 block">Description / Notes</label>
                    <textarea name="description" id="description" rows="2" placeholder="Enter notes or description..." class="w-full text-xs border border-slate-200 rounded-lg p-2 outline-none focus:border-indigo-500 transition-all">{{ old('description', $nsbReceipt->description) }}</textarea>
                </div>
                ...
                <div class="pt-1">
                    <button type="submit" class="w-full flex items-center justify-center gap-1.5 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg text-xs font-bold shadow transition-all duration-200">
                        <i class="bi bi-check-circle"></i> Update NSB Receipt
                    </button>
                </div>
            </form>
        </div>
