...
@section('page-content')
    <div class="flex flex-col space-y-6">
        ...
        <div class="flex flex-col md:flex-row md:items-start justify-between gap-4 py-2">
            <div>
                ...
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        <h1 class="text-xl font-bold text-slate-800 leading-none mb-2">Import Students</h1>
        ...
        <div class="bg-white rounded-[1rem] md:rounded-[1.5rem] border border-slate-100 shadow-sm overflow-hidden">
            ...
            {{-- Toolbar --}}
            <div class="px-4 md:px-8 py-5 border-b border-slate-50 bg-slate-50/40 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div class="flex items-center gap-3">
                    ...
                    <label for="chkAll"
                        ...
                        <span class="text-[10px] font-bold text-slate-600 uppercase tracking-widest group-hover:text-teal-600 transition-colors">All</span>
                    </label>
            ...
            <form action="{{ route('test-subject.import.store', $testSubject) }}" method="post" id="importForm">
                ...
                @if($missing->count())
                        ...
                        @foreach ($missing as $student)
                            <label for="stu-{{ $student->id }}"
                                ...
                                <div class="flex-1 min-w-0">
                                    <p class="student-name text-[10px] font-bold text-slate-800 leading-tight transition-colors">{{ $student->name }}</p>
                                    <p class="text-[9px] font-medium text-slate-400 mt-0.5">{{ $student->father_name }}</p>
                                </div>
                    ...
                    {{-- Submit footer --}}
                    <div class="px-4 md:px-8 py-6 border-t border-slate-50 bg-slate-50/40 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                        <p class="text-xs text-slate-500 font-medium">
                            <span id="footerCount" class="font-bold text-slate-800">0</span> student(s) will be enrolled in this subject test.
                        </p>
                        <button type="submit"
                                class="flex items-center gap-2 px-6 py-3 bg-teal-600 text-white rounded-xl text-[9px] font-bold uppercase tracking-widest hover:bg-teal-700 hover:shadow-lg hover:shadow-teal-100 transition-all whitespace-nowrap">
                ...
                @else
                    <div class="py-20 text-center">
                        ...
                        <a href="{{ route('test.test-subjects.show', [$testSubject->test, $testSubject]) }}"
                           class="inline-flex items-center gap-2 mt-6 px-5 py-2.5 bg-teal-600 text-white rounded-xl text-[9px] font-bold uppercase tracking-widest hover:bg-teal-700 transition-all">
