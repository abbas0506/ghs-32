        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 py-2">
            <div>
                ...
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        <h2 class="text-xl font-bold text-slate-800 leading-none mb-1">{{ $student->name }}</h2>
                        <p class="text-slate-400 text-xs font-medium italic">Individual graphical attendance analysis</p>
                    </div>
                </div>
            </div>
            <a href="{{ route('section.attendance.show', [$section->id, $attendance->id]) }}" class="px-6 py-3 bg-white border border-slate-200 text-slate-500 rounded-xl text-[10px] font-bold uppercase tracking-widest hover:bg-slate-50 transition-all shadow-sm flex items-center justify-center gap-2">
                ...
                <div class="bg-white rounded-[2rem] p-6 md:p-8 border border-slate-100 shadow-sm relative overflow-hidden flex flex-col">
                    ...
                    <div class="flex items-center justify-between mb-6 relative z-10 w-full shrink-0">
                        <div>
                            ...
                            <p class="text-[10px] font-bold text-slate-400 tracking-widest uppercase mt-1">{{ $student->name }} - Last 15 Days Log</p>
                        </div>
                    </div>
                                ...
                                <div class="flex flex-col items-center group relative cursor-pointer w-14">
                                    @if($day['status'] == 1)
                                        <div class="w-12 h-12 rounded-full border-4 border-white bg-teal-500 shadow-xl shadow-teal-500/30 flex items-center justify-center transition-transform group-hover:scale-110 mb-3 group-hover:-translate-y-1 relative z-10">
                                            <i class="bi-check text-white text-2xl font-bold"></i>
                                        </div>
                                        <span class="text-[9px] font-bold uppercase tracking-widest text-teal-600 opacity-100 transition-all text-center">Present</span>
                                    @elseif($day['status'] == 0)
                                        ...
                                        <span class="text-[9px] font-bold uppercase tracking-widest text-rose-500 opacity-100 transition-all text-center">Absent</span>
                                    @else
                                        ...
                                        <span class="text-[9px] font-bold uppercase tracking-widest text-slate-400 opacity-100 transition-all text-center">No Data</span>
                                    @endif
                                    <span class="text-[10px] font-bold text-slate-700 mt-1.5 align-middle leading-none">{{ $dayName }}</span>
