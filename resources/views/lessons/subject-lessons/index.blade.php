...
@section('page-content')
    ...
    <div class="flex items-center justify-between flex-wrap gap-3 mb-2">
        <div>
            ...
            <div class="bread-crumb mt-1">
                <a href="{{ url('/') }}">Home</a>
                <div>/</div>
                <a href="{{ route('lessons.index', ['grade_id' => $grade->id]) }}">{{ $grade?->name }}</a>
                <div>/</div>
                <span class="text-gray-600">{{ $subject->name }}</span>
                <div>/</div>
                <span class="text-gray-600">Lesson Plan</span>
            </div>
        </div>
    </div>
    ...
    <div class="md:w-11/12 mx-auto mt-6 space-y-6">
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
            <div
                class="flex items-center justify-between px-6 py-4 border-b border-gray-100 bg-gradient-to-r from-slate-50 to-gray-50">
                ...
                @if ($grade && $subject)
                    @role('head')
                        <form action="{{ route('grade.subject.lessons.destroy', [$grade, $subject, 1]) }}" method="POST"
                            id="deleteForm" class="inline">
                            @csrf
                            @method('DELETE')
                                ...
                                <table class="table-fixed borderless w-full text-sm xs md:sm">
                                    ...
                                    <tbody>
                                        @foreach ($lessons as $lesson)
                                            <tr class="tr">
                                                <td>
                                                    <a href="{{ route('grade.subject.lessons.edit', [$grade, $subject, $lesson->id]) }}"
                                                        class="ico cyan mx-auto">
                                                        {{ $lesson->lesson_no }}
                                                    </a>
                                                </td>
                                                <td class="hidden">{{ $lesson->title }}</td>
                                                <td class="text-left">
