...
@section('page-content')
    ...
    @php
        ...
        $similey = $percentage >= 90 ? 'smile' : ($percentage < 90 && $percentage >= 75 ? 'neutral' : 'angry');
        $themeColor = $percentage >= 90 ? 'green' : ($percentage < 90 && $percentage >= 75 ? 'cyan' : 'red');

    @endphp
    <!-- Section Summary Card -->
    <div class="w-full md:w-4/5 mx-auto bg-white mt-8 rounded-lg">
        <div class="statbox {{ $themeColor }}">
            <div class="flex justify-between items-center">
                ...
                @if (\Carbon\Carbon::parse($date)->isToday())
                    <a href="{{ route('section.attendance.edit', [$section, 1]) }}" aria-label="Edit attendance"
                        class="ico {{ $themeColor }} flex items-center">
