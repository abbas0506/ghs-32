...
@section('page-content')
    <div class="flex flex-col space-y-6">
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 py-2">
            <div>
                <div class="flex items-center gap-2 text-slate-400 text-[9px] uppercase tracking-[0.1em] font-bold mb-3">
                    ...
                    <span class="text-teal-600">Class Detail</span>
                </div>
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p class="text-slate-400 text-[10px] font-medium italic">{{ \Carbon\Carbon::parse($date)->format('l, d M Y') }}</p>
                    </div>
                </div>
            </div>
            ...
            @if (\Carbon\Carbon::parse($date)->isToday())
                <a href="{{ route('section.attendance.edit', [$section, 1]) }}" 
                   class="flex justify-center items-center gap-2 px-6 py-3 bg-teal-600 text-white rounded-[0.8rem] md:rounded-[1rem] text-[10px] font-bold uppercase tracking-widest hover:bg-teal-700 hover:shadow-lg hover:shadow-teal-100 transition-all">
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div class="bg-teal-600 rounded-[1rem] md:rounded-[1.5rem] p-5 text-white shadow-xl shadow-teal-100 flex items-center justify-between col-span-2 relative overflow-hidden">
            ...
            <div class="bg-white rounded-[1rem] md:rounded-[1.5rem] p-4 md:p-6 border border-slate-100 shadow-sm flex items-center justify-between">
                ...
                <div class="w-8 h-8 md:w-12 md:h-12 rounded-2xl bg-teal-50 text-teal-600 flex items-center justify-center">
                    <i class="bi-person-check text-lg md:text-2xl"></i>
                </div>
            </div>

            <div class="bg-white rounded-[1rem] md:rounded-[1.5rem] p-4 md:p-6 border border-slate-100 shadow-sm flex items-center justify-between">
                <div>
                    <p class="text-[9px] md:text-[10px] font-bold text-slate-400 uppercase tracking-wide mb-1">Absent</p>
                    <h2 class="text-lg md:text-2xl font-bold text-rose-600 leading-none">
                        {{ $absentCount }}
                    </h2>
                </div>
                <div class="w-8 h-8 md:w-12 md:h-12 rounded-2xl bg-rose-50 text-rose-600 flex items-center justify-center">
                    <i class="bi-person-x text-lg md:text-2xl"></i>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-[1rem] md:rounded-[1.5rem] border border-slate-100 shadow-sm overflow-hidden">
            ...
            <div class="overflow-x-auto">
                <table class=" table-fixed w-full border-collapse">
                    <thead>
                        <tr class="">
                            <th class="w-12 text-[10px] font-bold text-slate-400 uppercase tracking-widest border-b border-slate-50">#</th>
                            <th class="w-36 text-[10px] font-bold text-slate-400 uppercase tracking-widest border-b border-slate-50">Student Profile</th>
                        </tr>
                    </thead>
                            ...
                            <tr class="tr group hover:bg-slate-50/80 transition-all {{ $isStudentPresent ? 'present' : 'absent' }}">
                                ...
                                <td class="px-4 py-3">
                                    <div class="flex flex-col items-start hover:cursor-pointer" onclick="window.location.href='{{ route('section.attendance.show', [$section, $attendance]) }}'">
                                        <a href="{{ route('section.attendance.show', [$section, $attendance]) }}" class="text-[10px] font-bold tracking-wide text-left">
                                            {{ $attendance->student->name }}
                                        </a>
                                        <div class="flex flex-col pointer-events-none">
                                            <span class="text-[9px] text-slate-400 tracking-wide truncate w-full">
                                                {{ $attendance->student->father_name }}
                                            </span>
                                            <span class="text-[9px] text-slate-400 uppercase flex items-center gap-1.5 tracking-wider">
                                                <i class="bi bi-telephone text-[9px] text-slate-400 bg-slate-50 px-1 "></i> {{ $attendance->student->phone ?? 'N/A' }}
                                            </span>
                                        </div>
                                    </div>
                                </td>
