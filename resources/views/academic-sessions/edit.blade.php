    ...
    <div class="space-y-6 pb-6">
        ...
        <div class="flex items-center justify-between py-2 border-b border-slate-100 pb-3">
            <div>
                ...
                <h1 class="text-xl font-bold text-slate-800 tracking-tight leading-none">Edit Session</h1>
            </div>
        ...
        <div class="bg-white rounded-2xl border border-slate-100 p-6 shadow-[0_4px_20px_rgba(0,0,0,0.01)] max-w-xl">
            <form action="{{ route('academic-sessions.update', $session->id) }}" method="POST" class="space-y-5">
                @csrf
                @method('PUT')
                ...
                <div class="space-y-1.5">
                    ...
                    <input type="text" name="name" id="name" value="{{ old('name', $session->name) }}" placeholder="e.g. 2025-2026" class="w-full px-4 py-2.5 rounded-xl border border-slate-200 text-sm font-semibold text-slate-700 placeholder-slate-400 focus:outline-none focus:border-indigo-500 transition-all" required>
                </div>
                ...
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <div class="space-y-1.5">
                        ...
                        <input type="date" name="start_date" id="start_date" value="{{ old('start_date', $session->start_date->format('Y-m-d')) }}" class="w-full px-4 py-2.5 rounded-xl border border-slate-200 text-sm font-semibold text-slate-700 focus:outline-none focus:border-indigo-500 transition-all" required>
                    </div>

                    <!-- End Date -->
                    <div class="space-y-1.5">
                        <label for="end_date" class="block text-[10px] font-bold text-slate-400 uppercase tracking-wider">End Date</label>
                        <input type="date" name="end_date" id="end_date" value="{{ old('end_date', $session->end_date->format('Y-m-d')) }}" class="w-full px-4 py-2.5 rounded-xl border border-slate-200 text-sm font-semibold text-slate-700 focus:outline-none focus:border-indigo-500 transition-all" required>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <div class="space-y-1.5">
                        ...
                        <input type="number" name="ftf_start" id="ftf_start" value="{{ old('ftf_start', $session->ftf_start) }}" min="0" class="w-full px-4 py-2.5 rounded-xl border border-slate-200 text-sm font-semibold text-slate-700 focus:outline-none focus:border-indigo-500 transition-all" required>
                    </div>
                    ...
                    <div class="space-y-1.5">
                        ...
                        <input type="number" name="nsb_start" id="nsb_start" value="{{ old('nsb_start', $session->nsb_start) }}" min="0" class="w-full px-4 py-2.5 rounded-xl border border-slate-200 text-sm font-semibold text-slate-700 focus:outline-none focus:border-indigo-500 transition-all" required>
                    </div>
                </div>

                <!-- Is Current Session Toggle -->
                <div class="flex items-center gap-3 py-2">
                    <input type="checkbox" name="is_current" id="is_current" value="1" {{ old('is_current', $session->is_current) ? 'checked' : '' }} class="w-4 h-4 text-indigo-600 border-slate-300 rounded focus:ring-indigo-500">
