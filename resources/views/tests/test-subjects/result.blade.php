...
@section('page-content')
    <h2>Class {{ $testSubject->section->name }}</h2>
    <div class="bread-crumb">
        <a href="{{ url('/') }}">Home</a>
        <div>/</div>
        <a href="{{ route('tests.index') }}">Tests</a>
        <div>/</div>
        <a href="{{ route('test.test-subjects.index', $testSubject->test) }}">Subjects</a>
        <div>/</div>
        <a href="{{ route('test-subject.results.index', $testSubject) }}">Result</a>
        <div>/</div>
        <div>Edit</div>
    </div>
        ...
        <form action="{{ route('test-subject.results.update', [$testSubject, 1]) }}" method="post"
            ...
            <div class="flex flex-wrap items-center gap-3 mt-8">
                <div class="flex items-center flex-wrap gap-3">
                    <div class="pill green"> <i class="ri-user-line mr-2"></i>{{ $testSubject->section->name }}</div>
                    <i class="bi-arrow-right mx-1"></i>
                    <div class="pill indigo"><i class="bx bx-book mr-2"></i>{{ $testSubject->subject->name }}</div>
                </div>
                <div class="flex space-x-3 items-center">
                    <div class="ico rose"><i class="bi-hand-index rotate-90"></i></div>
                    <h3 class="text-red-600">Maximum Marks </h3>
                    <input type="number" id="max_marks" name="max_marks" value="{{ $testSubject->max_marks }}"
