    ...
    <div class="flex flex-col space-y-6">
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 py-2">
            <div>
                ...
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        <h1 class="text-2xl font-bold text-slate-800 leading-none mb-1">{{ $section->name }}</h1>
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div class="bg-teal-600 rounded-3xl p-6 text-white shadow-xl shadow-teal-100 flex items-center justify-between col-span-2 relative overflow-hidden">
                ...
                <div>
                    <p class="text-[10px] font-bold text-teal-100 uppercase tracking-widest mb-1">Attendance Rate</p>
                    <h2 class="text-4xl font-bold text-white">{{ $percentage }}%</h2>
                </div>
            ...
            <div class="bg-white rounded-2xl md:rounded-3xl p-4 md:p-6 border border-slate-100 shadow-sm flex items-center justify-between">
                <div>
                    <p class="text-[9px] md:text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">Present</p>
                    <h2 class="text-2xl md:text-3xl font-bold text-slate-800 leading-none">
                        {{ $presentCount }}
                    </h2>
                </div>
            ...
            <div class="bg-white rounded-2xl md:rounded-3xl p-4 md:p-6 border border-slate-100 shadow-sm flex items-center justify-between">
                <div>
                    <p class="text-[9px] md:text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">Absent</p>
                    <h2 class="text-2xl md:text-3xl font-bold text-rose-600 leading-none">
                        {{ $absentCount }}
                    </h2>
                </div>
        ...
        <div class="bg-white rounded-[2rem] border border-slate-100 shadow-sm overflow-hidden">
            ...
            <div class="px-4 md:px-8 py-4 md:py-6 bg-slate-50/50 border-b border-slate-100 flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div class="flex items-center flex-wrap gap-1 bg-white p-1 rounded-xl shadow-sm border border-slate-100 w-full md:w-auto">
                    ...
                    <button onclick="filterBy('present')" class="flex-1 md:flex-none px-3 md:px-5 py-2 rounded-lg text-[10px] md:text-xs font-bold uppercase tracking-widest cursor-pointer bg-teal-50 text-teal-700 hover:bg-teal-100 transition-all flex items-center justify-center gap-1.5"><i class="bi-person-check text-sm block -mt-0.5"></i> Present</button>
                    <button onclick="filterBy('absent')" class="flex-1 md:flex-none px-3 md:px-5 py-2 rounded-lg text-[10px] md:text-xs font-bold uppercase tracking-widest cursor-pointer bg-rose-50 text-rose-700 hover:bg-rose-100 transition-all flex items-center justify-center gap-1.5"><i class="bi-person-x text-sm block -mt-0.5"></i> Absent</button>
                </div>
            ...
            <div class="overflow-x-auto">
                <table class=" table-fixed w-full border-collapse">
                    <thead>
                        <tr class="">
                            <th class="w-16 px-5 py-2 text-[10px] font-bold text-slate-400 uppercase tracking-widest border-b border-slate-50">Roll #</th>
                            <th class="w-48 px-5 py-2 text-[10px] font-bold text-slate-400 uppercase tracking-widest border-b border-slate-50">Student Profile</th>
                            <th class="w-32 px-5 py-2 text-[10px] font-bold text-slate-400 uppercase tracking-widest border-b border-slate-50">Status</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-slate-50">
                        @foreach ($attendances as $attendance)
                            ...
                            <tr class="tr group hover:bg-slate-50/80 transition-all {{ $isStudentPresent ? 'present' : 'absent' }}">
                                <td class="py-2">
                                    <div class="w-10 h-10 rounded-full flex items-center mx-auto justify-center text-xs font-bold uppercase transition-all shadow-sm border {{ $isStudentPresent ? 'bg-teal-50 text-teal-700 border-teal-100 group-hover:bg-teal-100' : 'bg-rose-50 text-rose-700 border-rose-100 group-hover:bg-rose-100' }}">
                                ...
                                <td class="px-4 py-3">
                                    <div class="flex flex-col items-start hover:cursor-pointer" onclick="window.location.href='{{ route('section.attendance.show', [$section, $attendance]) }}'">
                                        <a href="{{ route('section.attendance.show', [$section, $attendance]) }}" class="text-xs font-bold text-left text-teal-600 underline decoration-teal-600/30 underline-offset-4 hover:text-teal-700 hover:decoration-teal-600 transition-all leading-tight pb-0.5">
                                ...
                                <td class="py-2">
                                    <div class="flex justify-center">
                                        @if ($isStudentPresent)
                                            <span class="px-3 py-1 bg-teal-50 text-teal-700 text-[10px] font-bold uppercase tracking-widest rounded-full border border-teal-100 shadow-sm">Present</span>
                                        @else
                                            <span class="px-3 py-1 bg-rose-50 text-rose-700 text-[10px] font-bold uppercase tracking-widest rounded-full border border-rose-100 shadow-sm">Absent</span>
                                        @endif
                                    </div>
                                </td>
        ...
        <div class="flex items-center justify-center pt-8">
            <a href="{{ route('attendance.summary') }}" class="flex items-center gap-2 px-8 py-3 rounded-2xl text-xs font-bold text-slate-500 hover:bg-slate-100 transition-all uppercase tracking-widest">
